...
## ADR-028 video-editing-skill は typed edit request の process boundary で接続し、agent は FFmpeg / ffmpeg-skill を直接扱わない
- 事実: video-editing-skill 0.1.0 が `contract --json` / `doctor --json` / `plan - --json` / `run - --json` を公開し、typed edit request（sources / operations / outputs）を ffmpeg-skill 経由で決定的に実行して output を検証、sha256 / timeline / OBSERVED probe / 実行 command を provenance として返す。ffmpeg-skill 0.9.x が唯一の engine で、agent 側の対応範囲（< 0.10）は PR #17 のまま。
- 決定: 責務境界は `video-production-agent（何をすべきか）→ VideoEditingAdapter（typed args → edit request、response → ToolResult）→ video-editing-skill（何を編集するか、検証、provenance）→ ffmpeg-skill（FFmpeg をどう実行するか）→ FFmpeg`。agent は command / argv / filter / executable / env を生成も転送もせず、Skill を import しない。Skill が返す `commands` は provenance として保存するだけで、再実行・編集の経路は作らない。
- Contract が source of truth: tools / operations / parameters / capabilities / unsupported / errors は `contract --json` から取り、`check_contract` に通らない installation は使わない（silent fallback 無し）。pinned snapshot（`contract_0.1.0.json`）と live contract の一致、contract の operation type と adapter の lowering の一致をテストで固定し、Skill 側の変更は agent 側で fail loudly する。
- Lowering は意味を変換する: Project IR の `video.trim {asset, keep}`（compiler の `input / segments / accurate / output`）を `CUT {keep: [{start, end}], precision}` へ。emit する parameter は contract の allowlist と照合し、無い parameter は転送しない。contract が宣言しない operation（CROP / FREEZE / REVERSE / IMAGE_INSERT / POSITION）は adapter が `UNSUPPORTED` として拒否し、agent が FFmpeg で代替しない。IR の語彙は変えない（`video.trim` のまま）。
- 選択は SkillRegistry のまま: `silence_cleanup` の候補は `["ffmpeg-skill/cut", "video-editing/cut"]`（Reference Skill が第一候補）。`select_tool` は候補 tool の package / ToolSpec の capability が明示的に MISSING のときだけその候補を飛ばす（UNKNOWN は validator の warning、推測しない）。planner / compiler / decision engine / QA は無変更。
- PathPolicy は二重に効く: agent の `PathPolicy`（allowed roots / workspace / output ≠ input）で検査した後、同じ workspace と roots を Skill の `--workspace` / `--allowed-input` として CLI flag で固定する（request では上書き不可）。出力 path は workspace 相対で渡す。
- 成功の定義は Skill の response 検証（`ok`、schema / skill id / version、delivered output の path 一致・存在・sha256、timeline、OBSERVED observation、operation provenance）を通ったときのみ。process exit 0 だけ、output が存在するだけ、malformed / 複数 document の stdout は失敗。構造化 error（code / retryable）は `recovery.classify_error` が stderr 文字列より先に読み、`TOOL_ERROR`（retryable）→ 有限再試行、非 retryable → BLOCK、`OUTPUT_ERROR` / `VALIDATION_ERROR` → `OUTPUT_INVALID`（再試行しない）、`CANCELLED(timeout)` → TIMEOUT、request 側の誤り → `INVALID_ARGS` / `INPUT_MISSING`。
- Artifact は既存の `_register_artifacts`（agent 自身の sha256）で登録し、Skill の OBSERVED probe は `provenance.skill_observations` に Observation 形式（skill / skill_version / tool / source / fingerprint）で記録する。inference / decision には戻さない。
- 追加しないもの: 新しい実行 engine、parallel execution、Decision Engine への tool 詳細、`video.concat` 等の IR 語彙（adapter は contract の全 operation を lower できるが、planner が emit するのは既存語彙だけ）。
