...
ENCODERS = ["libx264", "libx265", "prores_ks", "libaom-av1", "libsvtav1", "h264_nvenc", "hevc_nvenc", "h264_videotoolbox", "hevc_videotoolbox", "h264_vaapi", "h264_qsv", "aac"]
...
FILTERS = {"libass": ["subtitles", "ass"], "zimg": ["zscale"], "tonemap": ["tonemap"], "loudnorm": ["loudnorm"], "scdet": ["scdet"],
           ...
           "xfade": ["xfade"], "acrossfade": ["acrossfade"]}   # required_capabilities named by video-editing-skill's contract (transitions)
...
class CapabilityResolver:
    ...
    def resolve(self, refresh: bool = False) -> Dict[str, Capability]:
        ...
        # which asks ffmpeg-skill for ffmpeg / ffprobe readiness; the agent never probes the engine on the Skill's behalf
        ve = locate_video_editing(self.video_editing_dir, self.env)
        if ve:
            try:
                ad = VideoEditingAdapter(ve, ffmpeg_skill_dir=str(skill.root) if skill else None, timeout=60.0)
                doc = ad.doctor()
                st = "AVAILABLE" if doc.get("ok") else "MISSING"
                caps["video-editing"] = Capability("video-editing", st, f"{ad.version} at {ve.describe()} (doctor {'ok' if doc.get('ok') else 'not ready'})",
                                                   {"version": ad.version, "root": ve.describe(), "contract": ad.contract.get("schema"), "tools": sorted(ad.tools),
                                                    ...
                                                    "execution": ad.contract.get("execution", {}).get("mode"), "engine": (ad.contract.get("engine") or {}).get("id"),
                                                    "doctor": {c.get("check"): c.get("status") for c in doc.get("checks") or []}, "problems": list(doc.get("problems") or [])})
            except Exception as e:  # noqa: BLE001 — an incompatible or broken installation is reported, never used
