...
def build_plan(decisions: List[Decision], analysis: AnalysisResult, tools: Dict[str, str], version: int = 1, frame_accurate: bool = False) -> Dict[str, Any]:
    ...
    for asset in analysis.assets:
        ...
        if dec_ids and end > start:
            ...
            steps.append({"id": f"step_trim_{asset.id}", "skill": "silence_cleanup", "tool": tool_for("silence_cleanup"), "decision_ids": dec_ids, "params": {"asset": asset.id, "keep": [[round(start, 3), round(end, 3)]]}})
        ...
        for d in decisions:
            if d.subject == "audio.loudness" and d.params.get("asset_id") == asset.id and d.decision.startswith("normalize"):
                ...
                steps.append({"id": f"step_loudness_{asset.id}", "skill": "loudness_normalization", "tool": tool_for("loudness_normalization"), "decision_ids": [d.id], "params": {"asset": asset.id, "target_lufs": d.params["target_lufs"], "true_peak": d.params["true_peak"]}})
    ...
    for d in decisions:
        if d.subject.startswith("delivery."):
            ...
            if t.get("preset"):
                steps.append({"id": f"step_export_{t['id']}", "skill": "delivery_export", "tool": tool_for("delivery_export"), "decision_ids": [d.id], "params": {"preset": t["preset"], "target": t["id"]}})
                steps.append({"id": f"step_check_{t['id']}", "skill": "delivery_check", "tool": tool_for("delivery_check"), "decision_ids": [d.id], "params": {"platform": t.get("platform", "custom"), "target": t["id"]}})
