...
installed console script `video-editing` on PATH. The agent never imports the package."""
...
def locate_video_editing(explicit: Optional[str] = None, env: Optional[Dict[str, str]] = None) -> Optional[VideoEditingSkill]:
    ...
    cands += [Path.home() / ".claude" / "skills" / "video-editing-skill", Path.cwd() / "vendor" / "video-editing-skill", Path.cwd().parent / "video-editing-skill"]
    for c in cands:
        found = _from_checkout(c)
        if found:
            return found
    exe = shutil.which("video-editing", path=env.get("PATH") if env is not os.environ else None)
    if exe:
