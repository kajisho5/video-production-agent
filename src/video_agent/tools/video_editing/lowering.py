"""Lowering: the agent's typed operation arguments → one video-editing request document.

The agent's Project IR speaks in operations (`video.trim {asset, keep, accurate}` today; the compiler hands the adapter
typed args such as {"input", "segments" | "keep", "accurate", "output"}). video-editing-skill speaks in an edit request
(sources / operations with allowlisted types and typed params / outputs relative to a workspace). This module translates
meaning, never syntax: every parameter it emits is checked against the parameter allowlist the Skill's own contract
declares for that operation type, so a contract change (a renamed or removed parameter) is refused here instead of being
forwarded blindly. Nothing here builds commands, argv, filters or paths outside the workspace.
"""
...
FORBIDDEN_ARG_KEYS = ("command", "commands", "argv", "cmd", "shell", "exec", "args", "script", "binary", "executable", "executables", "env", "environment",
                      ...
                      "workspace", "allowed_input", "allowed_inputs", "allowed_input_roots", "ffmpeg_skill_dir")
COMMON_KEYS = ("timeout",)
...
class Lowering:
    ...
    def __init__(self, contract: Dict[str, Any], workspace: str):
    ...
    def relative_output(self, path: str) -> str:
        """Output path relative to the Skill's workspace (== the agent's workspace); an output elsewhere is refused."""
        try:
            rel = os.path.relpath(str(Path(path).resolve()), self.workspace)
        except ValueError as e:   # different drive on Windows
            raise ToolError(f"video-editing: output must be inside the workspace {self.workspace}: {e}")
        if rel == "." or rel.startswith("..") or os.path.isabs(rel):
            raise ToolError(f"video-editing: output {path} is outside the workspace {self.workspace}")
        return rel.replace(os.sep, "/")

    def build_request(self, tool: str, args: Dict[str, Any], paths: Dict[str, str], op_id: str = "op", timeout: Optional[float] = None) -> Tuple[Dict[str, Any], str, List[str]]:
        ...
        req: Dict[str, Any] = {"schema": self.request_schema,
                               "project": {"id": safe_id(op_id), "sources": sources, "operations": [operation],
                                           "outputs": [{"id": "out", "operation": "edit", "path": self.relative_output(out_path)}]},
