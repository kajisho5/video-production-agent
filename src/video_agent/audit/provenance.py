...
def build_provenance(ir_doc: Dict[str, Any], ops: List[Operation], results: List[ToolResult], paths: Dict[str, str], recovery: List[Dict[str, Any]], qa: Dict[str, Any], who: str = "video-agent") -> Dict[str, Any]:
    ...
    for op in ops:
        ...
        entries.append({
            ...
            # facts an external Skill reported about its own execution (identity, output hash, timeline, OBSERVED probe): recorded, never re-run
            "skill_result": None if last is None or not isinstance(last.data, dict) or "skill" not in last.data else
                            {k: last.data.get(k) for k in ("skill", "status", "operation_id", "operation", "artifact", "timeline", "observation", "error") if k in last.data},
        })
